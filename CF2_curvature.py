...
if __name__ == '__main__':
    ...
    S = [1.0, 0.9, 0.95, 1.05, 1.1, 1.15, 1.2, 1.25, 1.3]
    n = 4
    ...
    if not done:
        ...
        if is_root:
            ...
            np.savez(f'cdfs_for_curvature_{RSD}_n={n}_z={z:.2f}{alphastr}', CDFs=CDFs, pCDFs=pCDFs, twoCDFs=twoCDFs, twopCDFs=twopCDFs, R=R, S=np.array(S), opt_b=np.array([opt_b]))
